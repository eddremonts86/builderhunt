...
import { AIDimensionMismatchError, AIProviderError } from '~/shared/lib/ai/errors'
...
describe('embedTexts', () => {
  it('throws AIEmbeddingUnavailableError when unconfigured', async () => {
    ...
    vi.resetModules()
    const { embedTexts: embedTextsUnconfigured } = await import('~/shared/lib/ai/embeddings')
    const { AIEmbeddingUnavailableError: AIEmbeddingUnavailableErrorFresh } = await import('~/shared/lib/ai/errors')
