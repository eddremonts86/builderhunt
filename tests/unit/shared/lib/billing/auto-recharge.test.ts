...
import { authUsers, billingAutoRechargeRules, billingCreditGrants, organizationMembers, organizations } from '~/shared/lib/db/schema'
import { createBillingCustomer, createBillingSubscription } from '~/shared/lib/repositories/billing'
import {
...
import { PACK_CATALOG } from '~/shared/lib/billing/catalog'
import { FakeBillingProvider } from '~/shared/lib/billing/fake-provider'
import type { CreateSetupIntentInput, SetupIntentResult } from '~/shared/lib/billing/provider'
...
describe('maybeTriggerAutoRecharge', () => {
  ...
  it('does not trigger, and does not pause, once the org has hit the payment-failure velocity threshold (§8 task 3)', async () => {
    const principal = await configuredOrg()
    const { PAYMENT_FAILURE_VELOCITY_THRESHOLD, recordPaymentFailure } = await import('~/shared/lib/billing/risk')
