...
`plans/implemented/07-responsive-mobile-design` (mobile nav pattern, floating-panel viewport clamping, table
overflow handling).
...
- Dashboard shell: fixed glass topbar; account/org controls always reachable. Below `md`, the
  ...
  `plans/implemented/07-responsive-mobile-design/spec.md` for the confirmed pattern) rather than silent
  horizontal scroll.
