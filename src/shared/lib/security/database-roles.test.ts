...
describe('database role migration', () => {
  it('defines distinct owner, web, worker, and readonly roles', async () => {
    ...
    for (const role of ['builderhunt_owner', 'builderhunt_app', 'builderhunt_auth', 'builderhunt_worker', 'builderhunt_readonly']) {
  ...
  it('makes runtime identities non-privileged and unable to bypass RLS', async () => {
    ...
    for (const role of ['BUILDERHUNT_APP', 'BUILDERHUNT_AUTH', 'BUILDERHUNT_WORKER', 'BUILDERHUNT_READONLY']) {
