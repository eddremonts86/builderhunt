 ...
 * credit-reservation/refund state (plans/phase-1/30-stripe-billing-platform/tasks.md §3).
...
/** `null` means unlimited — the canonical, typed replacement for a JS `Infinity` value JSON can never actually carry (`JSON.stringify(Infinity) === 'null'` regardless of intent) — plans/phase-1/30-stripe-billing-platform/tasks.md §9 task 1. */
...
/** Placeholder until plans/phase-1/30-stripe-billing-platform/tasks.md §9 task 4 ("Add verified billing contact management") lands — always `null` today; typed now so this DTO's shape doesn't need a second breaking change later. */
...
/**
 * The canonical organization billing DTO (plans/phase-1/30-stripe-billing-platform/tasks.md §9 task 1) — plan/
...
export async function getOrganizationBillingSummary(principal: TenantPrincipal): Promise<OrganizationBillingSummaryDto> {
  ...
  return {
    ...
    // Always null until plans/phase-1/30-stripe-billing-platform/tasks.md §9 task 4 lands — see BillingContactSummaryDto's own comment.
    billingContact: null,
    capabilities: {
...
/**
 * Read-only preview shown before confirming an ownership transfer (plans/phase-1/30-stripe-billing-platform/
