...
export function BrandLogoMark({ size = 22 }: { size?: number }) {
  return (
    <span
      ...
      style={{ width: size, height: size, background: 'linear-gradient(135deg, var(--color-bh-accent), var(--color-bh-accent-hover))' }}
      aria-hidden="true"
    >
      <svg width={size * 0.6} height={size * 0.6} viewBox="0 0 24 24" fill="none">
        ...
        <circle cx="11" cy="12" r="1.9" fill="var(--color-bh-cyan)" />
      </svg>
    </span>
  )
}
