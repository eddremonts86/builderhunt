...
import { CheckoutError, createSubscriptionCheckout, type CheckoutErrorCode } from '~/shared/lib/billing/checkout'
...
const CHECKOUT_ERROR_STATUS: Record<CheckoutErrorCode, number> = {
...
export const Route = createFileRoute('/api/billing/checkout/subscription')({
  component: () => null,
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          const parsed = SubscriptionCheckoutBody.safeParse(await request.json().catch(() => ({})))
          if (!parsed.success) {
            return Response.json({ error: 'Invalid body', issues: parsed.error.flatten() }, { status: 400 })
          }

          const provider = getBillingProvider()
          const result = await withTenantContext(principal, (tx) => createSubscriptionCheckout(tx, principal, parsed.data, { provider }))
          ...
          if (error instanceof CheckoutError) {
            return Response.json({ error: error.message, code: error.code }, { status: CHECKOUT_ERROR_STATUS[error.code] })
          }
