...
export function FAQSection() {
  return (
    <div className="space-y-3">
      {FAQ_ITEMS.map((item) => (
        <details key={item.q} className="group border-b border-bh-border pb-4">
          <summary className="flex cursor-pointer list-none items-center justify-between font-semibold text-bh-text outline-none focus-visible:ring-2 focus-visible:ring-bh-accent rounded-lg p-2">
            <span>{item.q}</span>
            <ChevronDown
