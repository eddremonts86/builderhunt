...
- No pgvector / embedding representation — the fingerprint stays a 5-metric + paradigm
  record compared with the existing pure `similarity()` function ([`semantic-search`](../22-semantic-search/spec.md)
...
Implemented in `src/lib/github/content.ts` (shared with [`work-sample`](../38-work-sample/spec.md),
...
- **Future (cut from this plan)**: global cross-user style search. Requires a global
  ...
  [`semantic-search`](../22-semantic-search/spec.md) is the precedent to extend, not a second
