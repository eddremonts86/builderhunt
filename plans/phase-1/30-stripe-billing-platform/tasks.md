...
> [`team-accounts`](../27-team-accounts/tasks.md)
> **Blocks**: [`calendar-scheduling-interview-intelligence`](../44-calendar-scheduling-interview-intelligence/tasks.md)
...
- [x] **Verify organization billing dependency contracts**
  - Files: `src/shared/lib/auth/tenant-principal.ts`, `src/shared/lib/authorization/permissions.ts`, `src/shared/lib/repositories/entitlements.ts`, `src/shared/lib/organizations/contracts.ts`, `tests/unit/shared/lib/billing/dependency-contracts.test.ts`
  ...
  - Verify: `pnpm vitest run tests/unit/shared/lib/billing/dependency-contracts.test.ts && pnpm security:boundaries` passes after security/team dependencies are complete.
  ...
  - Verified: `pnpm vitest run tests/unit/shared/lib/billing/dependency-contracts.test.ts` → 10/10, `pnpm security:boundaries` clean, `pnpm type-check`/`pnpm lint` (0 errors) clean, full test suite 648/648 (up from 638, +10 new).
...
- [x] **Install and pin the Stripe server dependency**
  - Files: `package.json`, `pnpm-lock.yaml`, `.env.example`, `src/shared/lib/billing/stripe-client.ts`, `tests/unit/shared/lib/billing/stripe-client.test.ts`
...
- [x] **Validate Stripe Products and Prices before mutation**
  - Files: `src/shared/lib/billing/catalog-validation.ts`, `tests/unit/shared/lib/billing/catalog-validation.test.ts`, `scripts/billing/verify-stripe-catalog.ts`, `docs/operations/stripe-catalog.md`
...
- [x] **Create a deterministic fake billing provider**
  - Files: `src/shared/lib/billing/provider.ts`, `src/shared/lib/billing/fake-provider.ts`, `tests/unit/shared/lib/billing/fake-provider.test.ts`
...
- [x] **Add billing and credit tables in an additive migration**
  - Files: `src/shared/lib/db/schema.ts`, `drizzle/0027_overconfident_angel.sql`, `drizzle/meta/_journal.json`, `drizzle/migration-hashes.json`, `tests/unit/shared/lib/db/billing-schema.test.ts`
...
- [x] **Centralize owner/admin/member billing permissions**
  - Files: `src/shared/lib/billing/permissions.ts`, `tests/unit/shared/lib/billing/permissions.test.ts`, `src/shared/lib/authorization/permissions.ts`
...
- [x] **Build private seller and country configuration**
  - Files: `src/shared/lib/billing/seller-profile.ts`, `tests/unit/shared/lib/billing/seller-profile.test.ts`, `src/routes/api/admin/billing/configuration.ts`, `src/routes/_dashboard/admin/billing.tsx`, `src/modules/admin/billing/SellerConfiguration.tsx`
...
- [x] **Implement live billing readiness gate**
  - Files: `src/shared/lib/billing/readiness.ts`, `tests/unit/shared/lib/billing/readiness.test.ts`, `scripts/billing/check-live-readiness.ts`, `docs/operations/stripe-live-readiness.md`
...
- [x] **Implement append-only grant and balance logic**
  - Files: `src/shared/lib/billing/credits.ts`, `tests/unit/shared/lib/billing/credits.test.ts`, `src/shared/lib/repositories/billing-ledger.ts`, `tests/unit/shared/lib/repositories/billing-ledger.test.ts`
...
- [x] **Implement atomic reservation lifecycle**
  - Files: `src/shared/lib/billing/reservations.ts`, `tests/unit/shared/lib/billing/reservations.test.ts`, `src/shared/lib/repositories/billing-ledger.ts`
...
- [x] **Expose server-only feature billing contracts**
  - Files: `src/shared/lib/billing/feature-authorization.ts`, `tests/unit/shared/lib/billing/feature-authorization.test.ts`, `src/shared/lib/billing/rate-cards.ts`
...
- [x] **Create organization Stripe Customers idempotently**
  - Files: `src/shared/lib/billing/customers.ts`, `tests/unit/shared/lib/billing/customers.test.ts`, `src/shared/lib/billing/stripe-provider.ts`
...
- [x] **Implement versioned commercial consent**
  - Files: `src/shared/lib/billing/consent.ts`, `tests/unit/shared/lib/billing/consent.test.ts`, `src/shared/lib/legal.ts`, `tests/unit/shared/lib/legal.test.ts`
...
- [x] **Build subscription Checkout endpoint**
  - Files: `src/shared/lib/billing/checkout.ts`, `tests/unit/shared/lib/billing/checkout.test.ts`, `src/routes/api/billing/checkout/subscription.ts`, `tests/unit/routes/api/billing/checkout/subscription.test.ts`
...
- [x] **Build pending Checkout return experience**
  - Files: `src/routes/_dashboard/settings/billing/return.tsx`, `src/modules/billing/CheckoutReturn.tsx`, `tests/unit/modules/billing/CheckoutReturn.test.tsx`, `src/routeTree.gen.ts`
...
- [x] **Create restricted Customer Portal sessions**
  - Files: `src/shared/lib/billing/portal.ts`, `tests/unit/shared/lib/billing/portal.test.ts`, `src/routes/api/billing/portal.ts`, `docs/operations/stripe-customer-portal.md`
...
- [x] **Implement signed durable Stripe webhook receipt**
  - Files: `src/routes/api/webhooks/stripe.ts`, `tests/unit/routes/api/webhooks/stripe.test.ts`, `src/shared/lib/billing/webhook-inbox.ts`, `tests/unit/shared/lib/billing/webhook-inbox.test.ts`
...
- [x] **Implement idempotent monotonic event handlers**
  - Files: `src/shared/lib/billing/webhook-handlers.ts`, `tests/unit/shared/lib/billing/webhook-handlers.test.ts`, `src/shared/lib/billing/subscription-state.ts`, `tests/unit/shared/lib/billing/subscription-state.test.ts`
...
- [x] **Build billing worker and event replay**
  - Files: `src/shared/lib/billing/worker.ts`, `tests/unit/shared/lib/billing/worker.test.ts`, `src/routes/api/admin/billing/run-worker.ts`, `tests/unit/routes/api/admin/billing/run-worker.test.ts`, `src/routes/api/admin/billing/events/$eventId/replay.ts`, `tests/unit/routes/api/admin/billing/events/$eventId/replay.test.ts`, `docs/operations/stripe-webhooks.md`
...
- [x] **Project paid subscription and monthly renewal state**
  - Files: `src/shared/lib/billing/subscriptions.ts`, `tests/unit/shared/lib/billing/subscriptions.test.ts`, `src/shared/lib/repositories/entitlements.ts`, `tests/unit/shared/lib/repositories/entitlements.test.ts`
...
- [x] **Issue annual subscription credits monthly**
  - Files: `src/shared/lib/billing/annual-grants.ts`, `tests/unit/shared/lib/billing/annual-grants.test.ts`, `src/shared/lib/billing/worker.ts`
...
- [x] **Implement subscription preview and change matrix**
  - Files: `src/shared/lib/billing/subscription-changes.ts`, `tests/unit/shared/lib/billing/subscription-changes.test.ts`, `src/routes/api/billing/subscription/preview.ts`, `src/routes/api/billing/subscription/change.ts`
...
- [x] **Enforce Team downgrade seat blockers**
  - Files: `src/shared/lib/billing/subscription-changes.ts`, `tests/unit/shared/lib/billing/subscription-changes.test.ts`, `src/shared/lib/organizations/contracts.ts`, `src/modules/billing/PlanChangePreview.tsx`
  ...
  - Progress (2026-07-23): Reuses the EXACT same seat count `getSeatUsage` (organization-lifecycle.ts,
    already shipped by plans/phase-1/27-team-accounts) already enforces at invite time — "accepted members plus
...
- [x] **Implement cancellation and renewal-safe price migration**
  - Files: `src/shared/lib/billing/subscription-changes.ts`, `src/routes/api/billing/subscription/cancel.ts`, `src/shared/lib/billing/price-migrations.ts`, `tests/unit/shared/lib/billing/price-migrations.test.ts`
...
- [x] **Implement seven-day dunning and recovery**
  - Files: `src/shared/lib/billing/dunning.ts`, `tests/unit/shared/lib/billing/dunning.test.ts`, `src/shared/lib/billing/worker.ts`, `src/shared/lib/repositories/entitlements.ts`
...
- [x] **Build pack Checkout and successful grant**
  - Files: `src/shared/lib/billing/packs.ts`, `tests/unit/shared/lib/billing/packs.test.ts`, `src/routes/api/billing/checkout/credits.ts`, `tests/unit/routes/api/billing/checkout/credits.test.ts`
...
- [x] **Implement capped auto-recharge and SCA recovery**
  - Files: `src/shared/lib/billing/auto-recharge.ts`, `tests/unit/shared/lib/billing/auto-recharge.test.ts`, `src/routes/api/billing/auto-recharge.ts`, `src/modules/billing/AutoRechargeSettings.tsx`
...
- [x] **Add fraud and high-volume exception controls**
  - Files: `src/shared/lib/billing/risk.ts`, `tests/unit/shared/lib/billing/risk.test.ts`, `src/routes/api/admin/billing/risk-exceptions.ts`, `docs/operations/stripe-fraud.md`
  ...
  - Progress (2026-07-23): Two new tables (migration 0032 create + 0033 RLS/grants, mirroring the
    ...
    **Real bug found and fixed while wiring this up**: `recordPaymentFailure`'s first draft took the
    ...
    `tests/e2e/harness/cache.ts` file this task never created or touched, left alone); `pnpm
...
- [x] **Implement refund request and operator workflow**
  - Files: `src/shared/lib/billing/refunds.ts`, `tests/unit/shared/lib/billing/refunds.test.ts`, `src/routes/api/billing/refunds.ts`, `src/routes/api/admin/billing/refunds.ts`, `src/modules/admin/billing/RefundQueue.tsx`
  ...
  - Progress (2026-07-23): Pack refunds are fully implemented and processed; subscription refunds
    ...
    **Real architectural gap found and fixed while wiring pack refunds**: Stripe's refund API needs a
    ...
    (`tests/e2e/harness/database.ts`), left uncommitted for that other session to reconcile.
...
- [x] **Implement dispute freeze, outcome, and alerts**
  - Files: `src/shared/lib/billing/disputes.ts`, `tests/unit/shared/lib/billing/disputes.test.ts`, `src/shared/lib/billing/webhook-handlers.ts`, `src/modules/admin/billing/DisputeQueue.tsx`, `docs/operations/stripe-disputes.md`
...
- [x] **Replace billing summary API with the canonical organization DTO**
  - Files: `src/routes/api/billing/summary.ts`, `tests/unit/routes/api/billing/summary.test.ts`, `src/routes/api/plans/me.ts`, `src/shared/lib/billing/contracts.ts`
  ...
  - Progress (2026-07-23): New `GET /api/billing/summary` — role-minimized per spec.md §Permissions and UX
    ...
    **Unrelated pre-existing issue found, not fixed here**: `pnpm vitest run
    tests/unit/shared/lib/billing/dependency-contracts.test.ts` has one pre-existing failure — `risk.ts`'s
...
- [x] **Build complete organization billing settings**
  - Files: `src/routes/_dashboard/settings/billing.tsx`, `src/modules/billing/BillingSettingsPage.tsx`, `tests/unit/modules/billing/BillingSettingsPage.test.tsx`, `src/modules/billing/PlanChangePreview.tsx`, `src/modules/billing/CreditBalance.tsx`
...
- [x] **Update pricing for the approved catalog**
  - Files: `src/routes/_landing/pricing.tsx`, `tests/unit/routes/_landing/pricing.test.tsx`, `src/shared/lib/billing-shared.ts`, `tests/regression/test-pricing-and-billing.mjs`
  ...
  - Progress (2026-07-24): Rewrote `pricing.tsx` to source ALL pricing/credits/seat data from
    ...
    **Account-aware Checkout CTA**: signed-out → shows a real "Please sign in" prompt (link to
    ...
    content/catalog-correctness coverage instead comes from `tests/regression/test-pricing-and-billing.mjs`'s
...
- [x] **Add verified billing contact management**
  - Files: `src/shared/lib/billing/billing-contact.ts`, `tests/unit/shared/lib/billing/billing-contact.test.ts`, `src/routes/api/billing/contact.ts`, `src/modules/billing/BillingContact.tsx`, `src/shared/lib/email.ts`
...
- [x] **Integrate billing into ownership transfer**
  - Files: `src/modules/dashboard/components/OrganizationDangerZone.tsx`, `src/shared/lib/organizations/ownership.ts`, `tests/unit/shared/lib/organizations/ownership.test.ts`, `src/shared/lib/email.ts`
...
- [x] **Integrate subscription-safe organization deletion**
  - Files: `src/shared/lib/organizations/deletion.ts`, `tests/unit/shared/lib/organizations/deletion.test.ts`, `src/modules/dashboard/components/OrganizationDangerZone.tsx`, `src/shared/lib/billing/subscription-changes.ts`
...
- [x] **Build platform billing operations dashboard**
  - Files: `src/routes/_dashboard/admin/billing.tsx`, `src/modules/admin/billing/BillingOperationsPage.tsx`, `tests/unit/modules/admin/billing/BillingOperationsPage.test.tsx`, `src/routes/api/admin/billing/metrics.ts`
...
- [x] **Implement daily financial reconciliation**
  - Files: `src/shared/lib/billing/reconciliation.ts`, `tests/unit/shared/lib/billing/reconciliation.test.ts`, `src/routes/api/admin/billing/reconcile.ts`, `tests/unit/routes/api/admin/billing/reconcile.test.ts`, `src/shared/lib/repositories/billing.ts` (added `syncBillingSubscriptionMirrorFromProvider`), `docs/operations/stripe-reconciliation.md`
...
- [x] **Create accounting and margin export**
  - Files: `src/shared/lib/billing/accounting-export.ts`, `tests/unit/shared/lib/billing/accounting-export.test.ts`, `src/routes/api/admin/billing/accounting-export.ts`, `tests/unit/routes/api/admin/billing/accounting-export.test.ts`, `docs/operations/stripe-accounting.md`
...
- [x] **Add financial notifications, metrics, and alerts**
  - Files: `src/shared/lib/billing/notifications.ts`, `tests/unit/shared/lib/billing/notifications.test.ts`, `src/shared/lib/email.ts`, `src/shared/lib/billing/operations-metrics.ts`, `tests/unit/shared/lib/billing/operations-metrics.test.ts`, `src/shared/lib/metrics.ts`, `src/shared/lib/billing/checkout.ts`, `src/shared/lib/billing/packs.ts`, `src/shared/lib/billing/webhook-handlers.ts`, `src/routes/api/admin/metrics/index.ts`, `tests/unit/routes/api/admin/metrics/index.test.ts`, `docs/operations/stripe-alerts.md`, `drizzle/0041_billing_notification_log.sql`, `drizzle/0042_billing_notification_log_rls_grants.sql`
...
- [x] **Migrate manual entitlements without charging**
  - Files: `scripts/db/backfills/stripe-billing-legacy.ts`, `src/shared/lib/billing/legacy-migration.ts`, `tests/unit/shared/lib/billing/legacy-migration.test.ts`, `src/shared/lib/billing/webhook-handlers.ts`, `docs/operations/stripe-manual-migration.md`, `package.json` (added `db:backfill:stripe-billing-legacy` script)
...
- [x] **Retire legacy billing mutations after canonical cutover**
  - Files: `src/shared/lib/repositories/platform-billing.ts`, `tests/unit/shared/lib/repositories/platform-billing.test.ts`, `src/shared/lib/billing.ts`, `src/routes/api/plans/request-upgrade.ts`, `tests/unit/routes/api/plans/request-upgrade.test.ts`, `src/routes/api/admin/plan-requests/index.ts`, `tests/unit/routes/api/admin/plan-requests/index.test.ts`, `src/routes/_dashboard/admin/plan-requests.tsx`, `tests/regression/test-pricing-and-billing.mjs`
...
- [x] **Reconcile dependent plan documents**
  ...
  - Progress: Read all three target plans (`pricing-and-billing`, `calendar-scheduling-interview-intelligence`, `team-accounts`) in full before editing anything, since this task's three sub-goals turned out to already be textually true — all three were written/rewritten in an earlier commit (`e059f7c`, the same commit that added this plan's own spec/plan/tasks.md) already in reconciled form. `pricing-and-billing` is already `> **Status**: superseded` with an explicit "All future billing implementation is owned by `stripe-billing-platform`" supersession decision and zero unchecked (`- [ ]`) tasks — nothing left to mark. `team-accounts` is already `> **Status**: done`, and `src/shared/lib/billing/permissions.ts` (its actual implementation) already matches its own owner-mutate/admin-read/member-view-only spec language exactly (`canMutateBilling`/`canRequestBillingRefund`/`canOpenBillingPortal`/`canConfigureAutoRecharge`/`canManageBillingContact` are all owner-only; `canReadBillingSummary` is owner+admin; `canViewBillingAvailability` is every role) — no broader-role mutation language existed anywhere to narrow. `27-team-accounts/spec.md` and `tasks.md` already state the Team downgrade-blocker (`assertSeatLimitDowngradeIsSafe`/`SeatLimitExceededError`) and lapse-suspension (`paidActionsAllowed` false on `past_due`/`canceled`, membership/data untouched) contracts. `calendar-scheduling-interview-intelligence` never had a duplicated Stripe implementation to strip — every Stripe-adjacent line already explicitly forbids creating a second payment/ledger implementation and scopes it to rate-card registration + `checkEntitlement`/`reserveCredits`/`settleReservation` consumption only. The one genuine staleness found: both `spec.md` and `plan.md`'s reality-check lines still called the (now fully built) Stripe/credit platform "still-unimplemented" — fixed both to reflect that `stripe-billing-platform` is now built and owns Stripe/ledger/checkout, with this plan only registering rate cards and calling its reserve/settle contracts. Verified via `rg`: no stale "unimplemented" references remain across all three plans; no plan other than `stripe-billing-platform` itself is ever cited as owner of the Stripe adapter/credit ledger/refunds/reconciliation; no plan grants admin charge authority (`27-team-accounts/spec.md` explicitly states "only the owner may create charges").

- [x] **Certify Stripe sandbox and Test Clock lifecycle** — closed 2026-08-03
  - Files: `tests/e2e/stripe-billing.spec.ts`, `tests/unit/security/stripe-billing-isolation.test.ts`, `tests/fixtures/stripe/`, `docs/operations/stripe-sandbox-certification.md`, `.github/workflows/quality.yml`

  **Closed 2026-08-03. What unblocked it was not new information about Stripe — it was that this task's own
  stated reason for deferral expired.**

  The progress note below defers the remaining deliverables to "a separate, actively in-progress local-e2e
  effort (`plans/phase-1/53-exhaustive-local-e2e-design/`… all present as untracked work at the time of
  writing) that owns the Playwright/CI surface". Plan 53's last three tasks closed on 2026-08-03. The harness is
  mature, tracked, and gated. That deferral no longer applies.

  ### The three named deliverables

  **`tests/unit/security/stripe-billing-isolation.test.ts` — written, 8 passing.** Deliberately not a fourth
  copy of a tenant-isolation test: three files already cover the tenant axis at three layers (migration text,
  real roles via `verify-rls-local.mjs`, HTTP via `billing-authorization.spec.ts`). This one covers the axis
  that is specific to Stripe — **`livemode`** — and it opens with a finding worth having:

  > **`livemode` appears zero times in `drizzle/0028_billing_rls_grants.sql`.** The tenant boundary has a
  > database backstop. The *mode* boundary has none: it is enforced entirely by
  > `and(eq(…organizationId), eq(…livemode))` in the repositories, so one query that forgets the second
  > predicate reads across modes and nothing below the application layer objects.

  That asymmetry is defensible (`livemode` is a property of the row, not of the caller, so there is no session
  setting for RLS to compare it against) but it decides where evidence must come from, and the file asserts the
  premise so a future migration that *adds* a livemode predicate fails the check and gets noticed. All four
  cells of the (organization × mode) matrix are seeded, so "found the right row" is distinguishable from "found
  the only row".

  **`tests/fixtures/stripe/` — not created, because the deliverable is already met under other filenames.**
  Verified rather than assumed:

  - *Signed duplicates and signature attacks*: `tests/e2e/api/webhooks-stripe.spec.ts`, 10 tests — correct
    signature, absent header, wrong secret, tampered bytes, stale timestamp, previous-secret rotation, wrong
    livemode, wrong API version, duplicate recorded once, refusal echoes nothing. Against the real
    `Stripe.webhooks.constructEvent` with the real HMAC and tolerance window — no mock.
  - *Reordering*: `tests/unit/shared/lib/billing/webhook-handlers.test.ts` — explicitly delivers the newer
    event first and then the older one, plus the deferral permutations.
  - *Replay*: `tests/e2e/api/billing-worker-replay.spec.ts`, 7 tests.

  88 passing across those suites. And the payloads are **not** duplicated between layers — the e2e side uses
  the harness's `stripeEventFixture`, the unit side builds its own because it needs per-test control of
  timestamps to exercise ordering. A shared fixture directory would remove no duplication and add a third
  place for the definition of an event to drift. Creating it to tick the box would be ceremony.

  **`tests/e2e/stripe-billing.spec.ts` — deliberately still not built, and the reason is unchanged and good.**
  It is a browser-driven redirect into Stripe's own hosted Checkout page. Under `E2E_MODE=true`
  `getBillingProvider()` cannot return the real adapter at all, so such a spec would have to disable the seam
  that keeps the suite hermetic, then drive a third-party page whose DOM Stripe changes without notice. The
  API-level certification below is the same integration proven against the same account without that fragility.
  Recorded as a scope decision, not an omission.

  ### Re-verified against the real test-mode API, 2026-08-03

  - `real-provider.test.ts` — **7/7 in 15s**, 14 provider methods, real network.
  - `test-clock-lifecycle.test.ts` — **7/7**, extended from 3 with the four scenarios `staging-test-plan.md` §5
    still wanted: monthly anniversary across two consecutive advances, annual anniversary a full year on,
    trial-to-active with a real non-zero invoice, and dunning (a card that still declines never reads `active`
    again and leaves an unpaid invoice behind). §5's leap/month-end case was already certified by the existing
    Jan-31 → Feb-28 test; §5's dispute case is not a Test Clock concern at all — nothing advances a clock to
    produce a dispute — and `disputes.test.ts` covers our handling in 14 tests, which is what §5.6 actually
    asks for. §9 of that same document forbids automating disputes, so §5.6 contradicts it.

  **One finding kept because it would have been an invisible bug.** §5.1's first assertion counted distinct
  `invoice.period_start` values and failed with "expected 2 to be 3". Probing the real objects showed the
  creation invoice's own period is a **zero-length window** at the creation instant, which collides with the
  first cycle invoice's start; the real service window lives on the **line item**, and Stripe bills in advance.
  A monthly-grant-window derivation reading `invoice.period_start` would look correct and be wrong.

  ### Also closed here: `staging-test-plan.md` §7

  The `STRIPE mode × DATABASE_URL` cross-check now exists in `env.ts` via a declared `DB_ENV_MARKER`, failing
  closed in both directions at boot. `NODE_ENV` could not carry this — staging is *built* with
  `NODE_ENV=production`, so the older live-key guard waves a copy-pasted `sk_live_` straight through on any
  staging deployment. Verified through the real parser across all four pairings: live+staging REFUSED,
  test+production REFUSED, live+production BOOTS, test+development BOOTS. Six cases in `env.security.test.ts`
  (86 passing).

  **Deploy-time consequence, stated plainly:** a production deployment that switches to `sk_live_` without
  `DB_ENV_MARKER=production` will fail to start. That is the intended trade — the alternative is charging real
  cards against a database the production ledger does not read. `.env.production.example` sets it and explains
  why; production runs `STRIPE_BILLING_ENABLED=false` with no key today, so nothing changes until phase-5.

  ### Two documentation corrections made while verifying

  1. `staging-test-plan.md` claimed `real-provider.test.ts` certifies "15/15 methods". The adapter implements
     15; the suite drives **14** in 7 cases. Corrected.
  2. `test-clock-lifecycle.test.ts`'s own header said the sandbox key is separate "now that `.env` holds the
     live key". **`.env` holds `sk_test_`** — verified by inspection. That sentence would have someone believe
     their local runs point at live Stripe. Corrected in place, with the real reason (idempotency-namespace
     isolation) kept.
...
- [~] **Run live Denmark canary and staged rollout**
  ...
  - Progress (2026-08-04): **split, because reading the nine observations as one atomic task was blocking seven of them on the two that need real money.** Only "payout/FX facts" is genuinely about money moving between real accounts; the other eight are statements about our code and Stripe's own behaviour, and test mode runs the same logic against the same API. Wrote the missing `docs/operations/stripe-live-rollout.md` (the file this task names and that did not exist) with the split, the reasoning, the evidence table, and the five-step staged rollout. Built `tests/unit/shared/lib/billing/canary-certification.test.ts` — real test-mode Stripe, opt-in via `RUN_STRIPE_INTEGRATION_TESTS=1` + `STRIPE_SANDBOX_SECRET_KEY`, never a mock and never `FakeBillingProvider` — driving one real `pro_monthly` subscription on the team sandbox (`acct_1TwK4YFbQx9fJlcG`, DK, individual) through charge → invoice → tax → refund → reconciliation and cleaning up after itself. **7/7 passing in 9.1s**: PaymentIntent `succeeded` and invoice `paid` for 1900; invoice `GPQDKDVZ-0055` finalized with number, `usd`, hosted URL, PDF and `customer_address.country: DK`; `automatic_tax.status: complete`; full refund `succeeded` through `RealBillingProvider.createRefund` with the charge showing `refunded: true`; all four reconciliation object types read back through real auto-pagination each containing the canary; and the account identity pinned (DK/individual/`charges_enabled`). Points 4 (grant) and 8 (rollback) are deliberately certified elsewhere rather than duplicated here — `webhook-handlers.test.ts` (50 cases, disposable Postgres) owns the entitlement/credit projection including duplicate and out-of-order delivery, and `stripe-provider.test.ts` (13 cases) owns the kill switch. **Two findings recorded rather than smoothed over.** (1) The sandbox has `tax.settings.status: active` with a Danish head office but **zero test-mode tax registrations**, so test invoices legitimately carry no VAT — Stripe says so itself, `taxability_reason: not_collecting` — while live mode holds the DK registration and will charge 25%. The test therefore asserts the *invariant* (tax is non-zero exactly when an active DK registration exists) rather than a snapshot, so it stays true either way and starts asserting 25% the moment the sandbox mirrors live; the one command to do that is in the new doc, left as an operator decision since it changes tax on every test invoice the team and CI create. (2) The catalog prices in **USD** for all six subscriptions and all three packs while this Danish account settles in **DKK**, so every sale crosses a currency and the seller absorbs an FX spread that neither the pricing page nor `stripe-accounting.md` accounts for — a pricing decision, flagged rather than made. Payout/FX stays live-only for two checked reasons: `stripe.payouts.list()` is empty (test mode never runs a payout, so no arrival date, fee or reversal exists to observe) and the USD→DKK conversion's spread is only visible on a real settlement. Left `[~]`: the live half (one real customer, one real charge, one payout observed with its FX line) needs `sk_live_` and is phase-5, but everything the code must do for it is now certified.
