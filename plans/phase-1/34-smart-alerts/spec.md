...
> detection" phase would consume [`unified-timeline`](../33-unified-timeline/spec.md) fetchers.
...
- `new_product` is effectively dead until a Product Hunt connector exists
  ([`producthunt-integration`](../18-producthunt-integration/spec.md) is still planned).

Real event detection (actual new repos, new posts) requires per-builder activity fetching —
that is exactly what [`unified-timeline`](../33-unified-timeline/spec.md) builds. Wiring the
