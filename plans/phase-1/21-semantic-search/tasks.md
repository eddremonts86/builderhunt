...
> **Status**: `complete`
> **Depends on**: [`security-and-multitenancy`](../01-security-and-multitenancy/tasks.md) (global-public identity/index classification and tenant-private query isolation); [`ai-expansion`](../20-ai-expansion/spec.md) (Phases 1–3: `embeddings.ts`, `minimax.ts`, `tasks.ts`, `budget.ts` must exist). Enhanced by [`proactive-discovery`](../22-proactive-discovery/spec.md) (optional cold-start seeding).
> **Blocks**: [`proactive-discovery`](../22-proactive-discovery/tasks.md) (hard)
...
- [x] **Document the production (Coolify) pgvector step**
  - Files: `plans/phase-1/21-semantic-search/plan.md` (Risks already cover it), `README.md` (deploy notes section)
