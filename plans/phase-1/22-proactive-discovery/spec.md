...
> **Depends on**: [`semantic-search`](../21-semantic-search/spec.md) (hard — this plan writes
...
> [`ai-expansion`](../20-ai-expansion/spec.md) — v1 makes zero LLM calls; embedding of the
> stubs this worker creates is drained by semantic-search's own worker.
> **Blocks**: nothing hard. Enhances [`semantic-search`](../21-semantic-search/spec.md)
> (cold-start fix — its spec already names this plan as the accelerator) and
> [`ai-sourcing-sprints`](../40-ai-sourcing-sprints/spec.md) reuses this worker's
> cell-cursor pattern.
