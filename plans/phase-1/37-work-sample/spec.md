...
> **Status**: `pending`
> **Depends on**: [`ai-expansion`](../20-ai-expansion/spec.md) (hard — task registry, `minimaxChat`, cache, budgets), [`code-fingerprinting`](../24-code-fingerprinting/spec.md) (soft — reuses the `src/lib/github/content.ts` fetch helpers; whichever plan ships first introduces that module)
> **Blocks**: nothing. **Supersedes**: [`technical-sandbox`](../38-technical-sandbox/spec.md) (merged into this plan — see its status header for rationale; its surviving kernel is this task's `suggestedInterviewQuestions` output).
