...
**`pnpm test:e2e:applications-free-path` verde** con `STRIPE_BILLING_ENABLED=false` y
`AI_DISABLED=true`; `pnpm security:boundaries` con las comprobaciones **1, 2, 5 y 6** de §Guardas
mecánicas activas desde este momento, no al final — incluida "sin envío externo" y
`check-forbidden-claims.mjs` añadido a `scripts/ci/local-quality.sh`.
...
duros pasan su corpus tabular incluida la semántica de `unknown_kept` y **la distinción knockout /
preferencia / housekeeping**, con el test de que un knockout con cualquiera de los dos lados
desconocido sigue siendo `unknown_kept`; `pnpm test:e2e:applications-free-path` ampliado al run.
...
`pnpm security:provider-metering` pasa sin añadir excepciones al allowlist; **test de componente que
afirma que `<FitBand band="high" requirements={[]} />` lanza `fit_band_requires_requirements`** — es el
harness que hasta ahora le faltaba al criterio de aceptación 9.
...
`application_kits` con `content_hash` generado y contenido INSERT-only, más
**`application_kit_claim_facts`**, la tabla de procedencia claim→fact que le da a la carta las mismas
cuatro capas de veracidad que tiene el CV (§8 del spec). Ensamblado del kit: CV
...
de release; **un `INSERT` en `application_kit_claim_facts` con un `fact_id` de otro tenant falla por
FK**, y borrar un `career_facts` enlazado desde una carta falla por `restrict`.
...
Exportación de cuenta con las ocho tablas, borrado duro con el orden de FK probado, retención
...
| 4 | El score de fit se lee como un veredicto sobre la persona | **Alta** | Alto: daño de producto directo al usuario, difícil de detectar porque no genera errores | Banda en vez de número por defecto; **`<FitBand>` recibe `requirements` como prop obligatoria y lanza si llega vacía**, con test de componente en el cierre de la fase 3 — no hay ningún otro componente que renderice `fit_band`; copia obligatoria de qué NO significa; `CHECK (fit_contested_at IS NULL OR (fit_band IS NULL AND rank IS NULL))` para la impugnación | Se acepta el coste de UI a cambio de que el número nunca aparezca desnudo. Se corrige un defecto: "la banda nunca sola" era una regla de composición que nadie verificaba, y el criterio de aceptación 9 no tenía harness. Ahora el invariante vive en el componente |
| 5 | La carta contiene una afirmación fabricada y el usuario la envía firmada | Media | Alto: consecuencia recae sobre el usuario, no sobre nosotros | **Cuatro capas, ya no dos**: `factIds.min(1)` en zod, `assertFactsAreConfirmed` contra `career_facts.status = 'confirmed'`, la tabla nueva **`application_kit_claim_facts`** con FK compuesta `restrict` a `career_facts(organization_id, id)` escrita en la misma transacción que el kit, y el gate de aprobación (`cover_letter_text` sin `UPDATE` para ningún rol, `status` no puede ser `ready` con blocker abierto). Más reintento y fallback sin carta | Se prefiere no generar carta antes que generar una plantilla plausible. **Se corrige un hueco real**: `cover_letter_fact_ids jsonb` no tenía FK ni tabla de enlace, así que la carta —el artefacto que la persona **firma y envía**— tenía dos capas donde el CV tiene cuatro, incumpliendo el contrato publicado #7 del plan hermano |
...
| 7 | Un admin de la organización ve la búsqueda de empleo de un miembro | Media | Alto: es exactamente el daño que la doble clave existe para evitar; podría costarle el empleo a alguien | Predicado RLS tenant **y** owner en las ocho tablas **para `builderhunt_app`**; permisos sin rama `elevated`; test negativo obligatorio que exige `404` y cuerpo vacío. **Corrección de una afirmación falsa de este plan**: las policies del **worker** (`job_applications_worker_select`, `application_events_worker_insert`) son **sólo tenant**, no la conjunción — lo que sostiene ese camino es `assertCareerOrganizationIsPersonal` (riesgo 12), una aserción de runtime, no la DDL | Se exige `404` y no `403`: un `403` confirma la existencia del recurso, que ya es una filtración. Y se escribe el hueco del worker en vez de dejarlo cubierto por una frase que no era cierta: quien lea "tenant y owner en todas las tablas" y borre la aserción del lease amplía el acceso en silencio |
...
| 11 | Se envía la fase 3 y nadie puede usarla porque `STRIPE_BILLING_ENABLED = false` | **Alta** (es el estado real hoy) | Medio: trabajo enviado en oscuro | Las fases 1 y 2 no dependen de IA ni de billing y son el producto lanzable hoy; el hecho está escrito en `spec.md` §Casos límite. **Con gate**: job de CI que corre el E2E de las fases 1–2 con `STRIPE_BILLING_ENABLED=false` y `AI_DISABLED=true` hasta `submitted_by_user`, incluido el import CSV | Se ordena a propósito el valor sin IA antes que el valor con IA. El gate es además el test más representativo del despliegue actual, porque ése **es** el estado de producción — su mantenimiento se paga solo |
...
| 18 | El producto **hereda el marketing del miedo al ATS** y vende el problema equivocado | Alta (circula por toda la categoría) | Medio-alto: la persona optimiza formato cuando su problema son 2.000 competidores por vacante, y las cifras son refutables públicamente | `spec.md` §Problema fija el dato real (**92 % de reclutadores no auto-rechaza por formato**) y los volúmenes por tipo de puesto; **comprobación 6 de §Guardas mecánicas**: `check-forbidden-claims.mjs` compartido con el plan hermano, donde vive la lista | Se prefiere decir la verdad incómoda ("hay 2.000 candidatos, no un robot que te odia") porque además es lo que hace útiles los knockouts y la recencia. La lista vive en un solo sitio a propósito: duplicarla la desincronizaría, y este dominio es donde más copia de esa clase se escribe |
