...
> **Status**: `pending`
> **Depends on**: [`semantic-search`](../../implemented/phase-1/22-semantic-search/tasks.md) (global `builder_embeddings` + pgvector HNSW — already shipped); [`proactive-discovery`](../../implemented/phase-1/23-proactive-discovery/tasks.md) (index breadth; already shipped — a thin index makes look-alikes weak but must not break them). Enhanced by [`collaboration-graph`](../collaboration-graph/spec.md) and [`availability-signals`](../availability-signals/spec.md) (neither is required).
> **Blocks**: nothing
