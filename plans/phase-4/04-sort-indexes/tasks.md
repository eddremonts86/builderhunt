...
> **Reality check**: 60 indexes exist in `src/shared/lib/db/schema.ts`; the next migration is `drizzle/0084`.
...
- [ ] **Declare the missing indexes and generate the migration**
  - Files: `src/shared/lib/db/schema.ts`, `drizzle/0084_*.sql`
...
- [ ] **Regenerate the migration hash manifest**
  ...
  - Verify: `pnpm test:migration-integrity` prints `{"valid":true,"migrations":85}`.
