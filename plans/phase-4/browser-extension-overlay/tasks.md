...
> **Status**: `pending`
> **Depends on**: [`security-and-multitenancy`](../../implemented/01-security-and-multitenancy/spec.md) (a new authenticated client outside the app's cookie/CSRF assumptions); [`ai-expansion`](../../implemented/21-ai-expansion/spec.md) (Chrome built-in AI is the local-first tier this surface sits closest to); [`ai-sourcing-sprints`](../../implemented/41-ai-sourcing-sprints/spec.md) (the "add to sprint" action target — already shipped). Binding: [`ai-policy`](../../_meta/ai-policy.md), [`security-policy`](../../_meta/security-policy.md).
> **Blocks**: nothing
...
- [ ] **Add the functional username index the cross-source lookup needs**
  ...
  - Do: Mint with `pnpm exec drizzle-kit generate --custom --name
    ...
    [`semantic-search`](../../implemented/22-semantic-search/spec.md)'s Coolify pgvector swap. Additive,
