...
> **Status**: `pending`
> **Depends on**: [`job-opportunities-workspace`](../job-opportunities-workspace/spec.md) (las **fases 7–8** lo necesitan de verdad — tailoring y lote consumen `job_opportunity_versions`; las fases 1–6 sólo necesitan que las tablas existan para las FK compuestas que la Fase 1 declara), [`ai-expansion`](../../implemented/phase-1/21-ai-expansion/spec.md), [`security-and-multitenancy`](../../implemented/phase-1/01-security-and-multitenancy/spec.md)
> **Blocks**: [`delegated-job-applications`](../delegated-job-applications/spec.md)
> **Reality check**: Extiende `src/shared/lib/db/schema.ts` (8 tablas nuevas), `src/shared/lib/ai/tasks.ts` (5 tasks), `src/shared/lib/billing/rate-cards.ts`, `src/shared/lib/billing-shared.ts`, `src/shared/lib/authorization/permissions.ts`, `src/shared/lib/repositories/account-privacy.ts`, `src/modules/dashboard/ui/shell/nav-config.ts`, `scripts/db/verify-api-isolation-local.mjs`, **`scripts/check-tenant-boundaries.mjs` (4 reglas nuevas — fichero que los tres planes de carrera editan, así que es hoy el de mayor probabilidad de conflicto)** y **`scripts/ci/local-quality.sh`**. Añade `scripts/check-forbidden-claims.mjs` y `scripts/baselines/career-identifiers.json` (new). Clona la forma RLS/grants de `drizzle/0085_candidate_documents_rls_grants.sql`, el patrón de worker de `src/routes/api/admin/alerts/run-worker.ts` y el contrato de storage de `src/lib/storage/types.ts`. **No** reutiliza `candidate_documents` (FK obligatoria a `candidate_submissions`; ver spec.md §Decisión sobre el foundation de documentos) y **no** implementa un segundo contrato de storage.
...
**Criterio de cierre**: `pnpm test:api-isolation:local` y `pnpm security:boundaries` verdes **con las
cinco comprobaciones nuevas de §Guardas mecánicas activas** (employer-side, `getCached`/`setCached`,
renderer, prefijo con su baseline en `scripts/baselines/career-identifiers.json`, y
`check-forbidden-claims.mjs` añadido a `scripts/ci/local-quality.sh`); el borrado duro de una cuenta con
perfil, hechos, documentos y CVs completa sin bloquearse en una FK.
...
`AI_DISABLED=true`; **`pnpm test:e2e:career-free-path` verde** con los tres flags del camino gratuito
(`CANDIDATE_UPLOADS_ENABLED=false`, `AI_DISABLED=true`, `STRIPE_BILLING_ENABLED=false`) sobre lo que
exista en esta fase.
...
`export_state = 'exportable'`; el PDF pasa extracción de texto; **los tres gates del renderer**
(opciones, HTML/CSP/escapado, y la regla 3 de §Guardas mecánicas) verdes; test de propiedad de
`validateResumeTruth` que afirma la correspondencia entre contadores y `content`, más el `409
verification_stale` cuando divergen; `pnpm test:e2e:career-free-path` ampliado a esta fase.
...
con `project management`; `pnpm test:e2e:career-free-path` ampliado con higiene y fidelidad.
...
| 7 | **El renderer PDF abre una superficie nueva** (ejecución, SSRF, exfiltración desde el texto de un hecho) | Media | Alto | Chromium ya presente (`Dockerfile:41`), `javaScriptEnabled: false`, `page.route('**', abort)`, CSP `default-src 'none'`, fuentes embebidas como `data:`, sandbox conservado, timeout 15 s, browser propio por render — **y ahora con gate**: `buildResumeRenderOptions()` extraído a un módulo puro, test unitario que afirma cada opción, test que afirma el escapado y la CSP, y regla 3 de §Guardas mecánicas que prohíbe `--no-sandbox` y `javaScriptEnabled: true` bajo `src/**/resumes/**` | **Cero dependencias nuevas.** DOCX se difiere al plan sucesor `resume-server-rendering`. Se corrige un defecto de este plan: la configuración de endurecimiento no tenía ni una prueba, así que borrar una línea en un refactor no rompía nada — el PDF seguía generándose y seguía pasando extracción de texto |
...
| 10 | **El foundation de documentos nunca se implementa** y el plan queda a medias | Media (sólo aterrizó el schema) | Medio | Las fases 1–5, 7 y 8 no dependen del upload; de la 4b sólo `measureParseFidelity` necesita el extractor, no el storage; la Fase 6 se aísla y, si hace falta, implementa los tres adaptadores contra el contrato ya existente. **Con gate**: `pnpm test:e2e:career-free-path` con `CANDIDATE_UPLOADS_ENABLED=false`, en el cierre de las fases 3, 4 y 4b | El plan es entregable y útil sin subir un solo archivo. La única pérdida es importar un CV viejo. Que siga siendo cierto **se ejecuta**, no se afirma: era una promesa de documento que nadie comprobaba |
| 11 | **Reutilizar `candidate_documents`** por presión de "no duplicar" | Media (el README lo sugiere) | Alto: `submission_id NOT NULL` lo hace imposible, y el cascade borraría CVs al borrar submissions ajenas | Tablas propias `career_documents`/`career_document_extractions` con el mismo diseño; la decisión y su evidencia quedan en spec.md; **regla 1 de §Guardas mecánicas** prohíbe importar `candidateDocuments`, `candidateSubmissions`, `pipeline*` y `organizationBuilders` desde el dominio career | Se reutiliza el **diseño** y el **contrato de tipos**, no las filas. El `NOT NULL` que hoy lo hace imposible pertenece a otra feature y podría cambiar; la regla de CI no depende de eso |
...
| 13 | **La caché filtra un CV entre tenants** | Media (la API genérica invita al fallo) | Crítico | **Regla 2 de §Guardas mecánicas**: `check-tenant-boundaries.mjs` prohíbe importar o llamar `getCached`/`setCached` bajo `src/**/{career,resumes}/**` y `src/routes/api/career/**`, en forma estática y dinámica (su clave `ai:cache:{taskId}:{hash}` no incluye la organización, `cache.ts:43`); obligatorio `tenantAiCacheKey` con `ownerUserId` en el input canónico | Un test unitario afirma que dos sujetos con input idéntico producen claves distintas. Se corrige un defecto: la prohibición era prosa y `getCached` sigue exportado, así que nada impedía usarlo. Ahora es un gate de CI |
...
| 15 | **Conflictos de merge** con los otros dos planes de carrera en 10 archivos compartidos (los 8 originales más `check-tenant-boundaries.mjs` y `scripts/ci/local-quality.sh`) | Alta | Bajo | Prefijos `career`/`resume` en todo identificador; superficies compartidas listadas en spec.md; **regla 4 de §Guardas mecánicas** afirma el prefijo sobre los identificadores exportados nuevos en `schema.ts`, `tasks.ts`, `rate-cards.ts` y `permissions.ts` | Los conflictos serán textuales, no semánticos. La regla es un grep con falsos negativos conocidos: su valor es cazar el copy-paste evidente en ficheros que tres planes tocan a la vez, no demostrar unicidad |
| 16 | **El plan free queda muerto** con `STRIPE_BILLING_ENABLED=false` y nadie puede autoupgradearse | Alta | Alto: la feature no se usa | El camino determinista completo (perfil, hechos, CV base, editor, validador, higiene, keywords, fidelidad de parseo, PDF, TXT, 3 versiones) es gratis. **Con gate**: `pnpm test:e2e:career-free-path` fija los tres flags a la vez y afirma que ninguna ruta responde 500 ni la UI ofrece una acción imposible; entra en el cierre de la Fase 3 y se amplía en 4 y 4b | Lo de pago es la asistencia de IA y el lote, no el producto. El mismo script cubre el riesgo 10: son el mismo fallo visto desde dos flags |
| 17 | **El checker reporta un problema que la persona no encuentra** y deja de confiar en el producto entero | Alta si no se diseña contra ello | Alto: es el fallo de producto mejor documentado de la categoría — el cofundador del competidor lo concedió públicamente ("alarmista sin ser específico", "una máquina tragamonedas en vez de un diagnóstico") | `.refine()` en el `anchor` de `hygieneFindingSchema` que rechaza los cuatro campos nulos a la vez (spec §El contrato), más el descarte en `evaluateResumeHygiene` como red de seguridad, más el test unitario del cierre de 4b | Se prefiere reportar menos y exacto antes que más y vago. El invariante va en el esquema y no en una convención: un check nuevo que no sepa anclar **no parsea**. La métrica "0 hallazgos sin anchor" detecta que el filtro está comiendo trabajo en silencio |
...
| 19 | **Se hereda el marketing del miedo al ATS** ("75 % rechazado automáticamente", "98,4 % de las Fortune 500", "6 segundos") | Alta (circula por toda la categoría y suena creíble) | Medio-alto: vende el problema equivocado y es refutable públicamente — la cifra del 75 % viene de un *pitch* de vendor de 2012 de una empresa cerrada en 2013 | Lista de copia prohibida en spec.md §Copia prohibida **más `scripts/check-forbidden-claims.mjs` (new) en `pnpm ci:local`**, que escanea `src/**`, copy y prompts, con allowlist para los dos documentos que explican por qué están prohibidas. El gate es compartido con el plan hermano: la lista vive en un sitio | El enemigo de la persona es el volumen (2.000+ candidaturas en tech remoto), no el parser. Decir la verdad aquí es además el posicionamiento más defendible. Una lista en un spec no la lee quien escribe la copia; un gate sí la para |
