...
| 10 | **El foundation de documentos nunca se implementa** y el plan queda a medias | Media (sólo aterrizó el schema) | Medio | Las fases 1–5 y 7–8 no dependen del upload; la Fase 6 se aísla y, si hace falta, implementa los tres adaptadores contra el contrato ya existente. **Con gate**: un job de CI corre el E2E de las fases 3, 4 y 4b con `CANDIDATE_UPLOADS_ENABLED=false` (el flag ya existe), igual que ya se corre con `AI_DISABLED=true` | El plan es entregable y útil sin subir un solo archivo. La única pérdida es importar un CV viejo. Que siga siendo cierto **se ejecuta**, no se afirma: era una promesa de documento que nadie comprobaba |
