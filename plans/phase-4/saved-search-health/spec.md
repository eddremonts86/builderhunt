...
> **Status**: `pending`
> **Depends on**: [`smart-alerts`](../../implemented/34-smart-alerts/spec.md) (`alerts` / `alert_triggers` are the signal source); [`ai-sourcing-sprints`](../../implemented/41-ai-sourcing-sprints/spec.md) (sprint results count as useful-match evidence). Both already have shipped code — see the reality check.
> **Blocks**: nothing
...
- **No cross-member roll-up.** An owner/admin does not get to see a colleague's saved-search
  health; org-visible saved searches are owned by [`shared-resources`](../../implemented/28-shared-resources/spec.md)
  (`blocked`).
...
| ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------- |
| "alerts fired" per saved search      | `alerts.query_id` exists (`schema.ts:397`) plus the composite tenant FK `alerts_organization_query_fk` (`schema.ts:439`) but **is written by nothing**: `CreateBody` in `src/routes/api/alerts/index.ts` has no `queryId`, `UpdateBody` in `src/routes/api/alerts/$id.ts` has no `queryId`, and `CreateOrganizationAlertInput` / `UpdateOrganizationAlertInput` have no `queryId` field. `grep -rn queryId src/ \| grep -v 'savedQueryId\|firstQueryId'` returns exactly two hits, both in `schema.ts`. | Dead column. Needs **wiring, no new column**.        |
...
2. **Latent bug this activates**: both FKs on `alerts.query_id`
   ...
   *Partially mitigated elsewhere, not fixed*: `hardDeleteAccountSubject`
   ...
   `saved_queries` to dodge exactly this FK (see `plans/implemented/04-legal-and-compliance/tasks.md`,
