...
- [ ] **Migrate APIs, workers, admin tools, exports, and deletion to tenant repositories**
  ...
  - Known gap, deliberately not fixed here (flagged, not silently passing): `loadAccountExportSource`'s `trackedBuilders` read hits the tenant-private `builders` table without `withTenantContext`, so RLS's `organization_id` policy silently returns zero rows — account export omits the subject's tracked builders. Needs `loadAccountExportSource` to loop the user's memberships and read `builders` once per organization under that organization's tenant context. Do not check this task off until this is fixed and the isolation script's presence check for it is restored.
