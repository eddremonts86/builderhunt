...
> [`proactive-discovery`](../proactive-discovery/spec.md) (soft — same HTTP-cron
> cell-cursor worker pattern, no code dependency);
> [`team-accounts`](../team-accounts/spec.md) (soft — shared sprints are a Future phase).
> **Blocks**: nothing.
...
- **No LLM grading of candidate code.** The old "vetting engine" overlaps
  [`code-fingerprinting`](../code-fingerprinting/spec.md) and
  [`work-sample`](../work-sample/spec.md); sprint result scoring is the search pipeline's
