...
> **Reality check**: 85 indexes exist in `src/shared/lib/db/schema.ts`; the next migration is the next free index at execution time (`drizzle/0115` today).
...
- [ ] **Declare the missing indexes and generate the migration**
  - Files: `src/shared/lib/db/schema.ts`, `drizzle/0115_*.sql` (the next free index `drizzle-kit generate` allocates)
