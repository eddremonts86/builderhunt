...
> **Reality check**: `src/shared/lib/db/schema.ts` declares 60 indexes, several already the composite this plan needs — `sprint_results_sprint_created_idx`, `billing_ledger_entries_org_created_idx`, `organization_plan_changes_org_created_idx`. Sorting by score, followers, name or status is unindexed everywhere. The last migration is `drizzle/0083`, so this one is `0084`. `drizzle/migration-hashes.json` is an immutability manifest regenerated with `node scripts/db/verify-migration-integrity.mjs --write`.
...
- `drizzle/0084_*.sql` contains only `CREATE INDEX` statements.
- `pnpm test:migrations:local` and `pnpm test:migration-integrity` green at 85 migrations.
