...
> `ENRICHMENT_ENABLED=false` everywhere. See plans/implemented/phase-1/42-stealth-scraping/task.md for exact gaps.)
> **Legacy directory name:** the `stealth-scraping` suffix is retained only to preserve existing
> links. The implementation and product copy must use **Public Profile Enrichment**. It must not
> claim stealth, anti-bot evasion, CAPTCHA solving, identity masking, or guaranteed access.
> **Depends on**: [`security-and-multitenancy`](../01-security-and-multitenancy/spec.md) for tenant
> context, worker role, and RLS, [`legal-and-compliance`](../04-legal-and-compliance/spec.md) for
> access/export/deletion, and the normalized `builder_identities` + `organization_builders` model.
