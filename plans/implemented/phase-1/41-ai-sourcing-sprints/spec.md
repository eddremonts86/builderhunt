...
> [`ai-expansion`](../21-ai-expansion/spec.md) (hard — task registry,
> `ai()` client, budgets); [`semantic-search`](../22-semantic-search/spec.md) (soft — sprint
> results write through `upsertEmbeddingStubs`; the feature works without it);
> [`proactive-discovery`](../23-proactive-discovery/spec.md) (soft — same HTTP-cron
> cell-cursor worker pattern, no code dependency);
> [`team-accounts`](../27-team-accounts/spec.md) (soft — shared sprints are a Future phase).
> **Blocks**: nothing.
...
- **No LLM grading of candidate code.** The old "vetting engine" overlaps
  [`code-fingerprinting`](../25-code-fingerprinting/spec.md) and
  [`work-sample`](../38-work-sample/spec.md); sprint result scoring is the search pipeline's
