...
> **Status**: `pending`
> **Depends on**: [`abuse-and-usage-integrity`](../../phase-1/31-abuse-and-usage-integrity/spec.md) (the decayed combined-signal scoring mechanics this plan reuses); [`claimable-profiles`](../../phase-1/35-claimable-profiles/spec.md) (a subject's explicit open-to-work state always outranks inference); [`legal-and-compliance`](../../phase-1/03-legal-and-compliance/spec.md) (inference about named individuals must be disclosed and contestable). Binding: [`security-policy`](../../_meta/security-policy.md), [`ai-policy`](../../_meta/ai-policy.md), [`conventions`](../../_meta/conventions.md).
> **Blocks**: nothing
> **Reality check**: The decay engine is shipped code (`src/shared/lib/abuse/risk.ts`, `drizzle/0043`–`0045`), the suppression mechanism is shipped (`builder_processing_restrictions`, `is_builder_processing_restricted`, `POST /api/me/builder/$builderId/restrict-processing` → `cascadeBuilderProcessingRestriction` at `src/lib/enrichment/worker.ts:206`), and the subject's own availability state is shipped (`published_builder_profiles.open_to_status`, `src/shared/lib/db/schema.ts:233`). `builder_source_snapshots` exists but still has **no runtime writer and no non-owner grants** at HEAD, so bio history must be created before any change-detection signal can work. An unrelated tenant-private "availability" domain (`availability_rules`/`availability_policies`/`availability_overrides`, `drizzle/0069`–`0071`) shipped after this plan was drafted — see `spec.md` §Naming for the prefixes that keep the two apart.
