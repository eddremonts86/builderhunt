...
> **Status**: `pending`
> **Depends on**: [`smart-alerts`](../../phase-1/33-smart-alerts/spec.md) (`alerts` / `alert_triggers` are the signal source); [`ai-sourcing-sprints`](../../phase-1/40-ai-sourcing-sprints/spec.md) (sprint results count as useful-match evidence). Both already have shipped code — see the reality check.
> **Blocks**: nothing
...
- **No cross-member roll-up.** An owner/admin does not get to see a colleague's saved-search
  health; org-visible saved searches are owned by [`shared-resources`](../../phase-1/27-shared-resources/spec.md)
  (`blocked`).
...
2. **Latent bug this activates**: both FKs on `alerts.query_id`
   ...
   *Partially mitigated elsewhere, not fixed*: `hardDeleteAccountSubject`
   ...
   `saved_queries` to dodge exactly this FK (see `plans/phase-1/03-legal-and-compliance/tasks.md`,
