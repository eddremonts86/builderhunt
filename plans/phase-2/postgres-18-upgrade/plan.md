...
1. **`FORCE ROW LEVEL SECURITY` + a non-superuser data-only restore silently lands zero rows.**
   58 tables are `FORCE`d and all seven `builderhunt_*` roles are `NOSUPERUSER … NOBYPASSRLS`, so
...
| `pgvector/pgvector:0.8.5-pg18` behaves differently from the floating `pg16` tag production runs today | Low | Medium | [`../README.md`](../README.md) (lines 209–211) already assumes pgvector 0.8.5 semantics; pinning makes it enforced. Phase 1 re-runs the HNSW `EXPLAIN` test and Phase 3 compares a real semantic-search result set before/after |
