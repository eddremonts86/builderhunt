...
> **Status**: `pending`
> **Depends on**: [`security-and-multitenancy`](../../phase-1/security-and-multitenancy/spec.md) (a new authenticated client outside the app's cookie/CSRF assumptions); [`ai-expansion`](../../phase-1/ai-expansion/spec.md) (Chrome built-in AI is the local-first tier this surface sits closest to); [`ai-sourcing-sprints`](../../phase-1/ai-sourcing-sprints/spec.md) (the "add to sprint" action target — already shipped). Binding: [`ai-policy`](../../_meta/ai-policy.md), [`security-policy`](../../_meta/security-policy.md).
> **Blocks**: nothing
...
- [ ] **Add the functional username index the cross-source lookup needs**
  ...
  - Do: Mint with `pnpm exec drizzle-kit generate --custom --name
    ...
    [`semantic-search`](../../phase-1/semantic-search/spec.md)'s Coolify pgvector swap. Additive,
