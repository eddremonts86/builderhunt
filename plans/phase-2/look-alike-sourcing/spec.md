...
- **No federated/keyword fallback.** "More like this" has no keyword form — the seed *is* a vector.
  ...
  [`ai-sourcing-sprints`](../../phase-1/40-ai-sourcing-sprints/spec.md) already does, so the thin-index state
...
Per `21-semantic-search/spec.md`'s own resolved edge cases, the same human is distinct
