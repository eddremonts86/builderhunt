...
`staging-test-plan.md` in `plans/implemented/phase-1/30-stripe-billing-platform/` (test-mode coverage), and
`stripe-incident-response.md` (the kill switch, i.e. rollback).

## Why this document is a split and not a checklist

`plans/implemented/phase-1/30-stripe-billing-platform/tasks.md` §15 asks for nine observations from the canary: a
